...
describe('calculatePairChemistry', () => {
  it('calculates chemistry for a pair with shift data', async () => {
    const shifts = [
  ...
  it('sorts player IDs canonically', async () => {
    const game = makeGame([], []);
    const result = await calculatePairChemistry([game], 500, 100, 10);
    expect(result.player1Id).toBe(100);
    expect(result.player2Id).toBe(500);
  });

  it('tracks shots against', async () => {
    const shots = [
...
describe('buildChemistryMatrix', () => {
  it('builds a matrix for given players', async () => {
...
describe('findChemistryExtremes', () => {
  it('returns best and worst arrays', async () => {
    const shots = Array.from({ length: 20 }, (_, i) =>
